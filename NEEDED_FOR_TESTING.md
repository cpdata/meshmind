...
## Python Runtime
- Python 3.11 or 3.12 is recommended; project metadata now pins `>=3.11,<3.13` because several dependencies (`pymgclient`,
...
- Core functionality relies on `openai`, `pydantic`, and `pydantic-settings`, but the repository ships a compatibility shim (`meshmind/_compat/pydantic.py`) that unlocks tests when Pydantic is unavailable.
- Optional packages improve specific workflows:
...
- **Graph backend** options:
  ...
  - **Neo4j** reachable via `NEO4J_URI` with credentials `NEO4J_USERNAME`/`NEO4J_PASSWORD` when the optional driver is installed.
    Use `meshmind admin graph --backend neo4j` to verify connectivity once credentials are configured.
...
- Recommended: Docker Compose (shipped in repo) to run Memgraph, Redis, and the Celery worker together when developing locally.
