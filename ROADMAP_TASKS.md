...
- **High-Level Task 1.4: Deliver MCP search and graph tooling parity**
  - **Subtask 1.4.a: Implement `/mcp/search/facts` endpoint** – Accept natural-language queries, translate to hybrid retrieval over memories and triplets, and return ranked facts; confirm via `tests/mcp/test_search_facts.py` and contract replay `examples/mcp/search_facts.http`.
  - **Subtask 1.4.b: Implement `/mcp/search/nodes` endpoint** – Support graph node lookup by label/attribute filters with pagination; verify via `tests/mcp/test_search_nodes.py` and `scripts/validate/search_nodes_benchmark.py` staying within p95 latency goals.
  - **Subtask 1.4.c: Provide `/mcp/episodes` management** – Add GET listing with filtering, DELETE endpoint for episode purges, and ensure soft-delete audit trail; confirm via `tests/mcp/test_episode_lifecycle.py` and audit log spot check.
  - **Subtask 1.4.d: Add `/mcp/graph/clear` administrative tool** – Require elevated auth, execute graph wipe with safeguard confirmation token, and emit event to `audit.graph.cleared`; verify via `tests/mcp/test_clear_graph.py`.
  - **Subtask 1.4.e: Expose `/mcp/graph/explain` path endpoint** – Return shortest and relevance-weighted paths between entities with natural language explanation; confirm via `tests/mcp/test_explain_path.py` and golden responses stored in `docs/mcp/explain_path_examples.md`.
- **High-Level Task 1.5: Ship chat-store compatibility layer**
  - **Subtask 1.5.a: Implement chat session persistence API** – Add `/chat/sessions` CRUD endpoints storing turn metadata and linking to memory IDs; verify via `tests/chat/test_sessions_api.py` and schema migration snapshot.
  - **Subtask 1.5.b: Provide conversation playback and export** – Deliver `/chat/sessions/{id}/transcript` and `/chat/sessions/{id}/export` supporting JSONL/Markdown formats; confirm via `tests/chat/test_transcript_export.py` and CLI command `meshmind-cli chat export`.
  - **Subtask 1.5.c: Document chat-store integration guide** – Author `docs/chat/chat_store_integration.md` mapping endpoints to LangChain/Zep adapters; verify via docs lint and partner review sign-off.
...
- **High-Level Task 2.4: Enable multi-agent shared memory spaces**
  - **Subtask 2.4.a: Model shared scope identifiers** – Extend schema to support crew/group scope keys persisted alongside tenant scopes; confirm via migration `alembic/versions/xxxx_shared_scopes.py` and schema tests.
  - **Subtask 2.4.b: Implement crew-sharing access controls** – Add policy layer `meshmind/security/shared_scopes.py` enforcing which agents can read/write shared memories; verify via `tests/security/test_shared_scope_policies.py`.
  - **Subtask 2.4.c: Provide shared memory orchestration helpers** – Extend SDKs and CLI with `--shared-scope` flags and helper classes enabling CrewAI/AutoGen style workflows; confirm via SDK integration tests and example notebooks `examples/shared_memory/crew_demo.ipynb`.
...
- **High-Level Task 6.4: Deliver conversation summary memory**
  - **Subtask 6.4.a: Implement summarization job** – Create `meshmind/jobs/conversation_summarizer.py` that rolls episodic turns into hierarchical summaries stored in `conversation_summaries` table; verify via `tests/jobs/test_conversation_summarizer.py`.
  - **Subtask 6.4.b: Expose summary retrieval endpoints** – Add `/chat/summaries/{session_id}` and `/chat/summaries/{session_id}/refresh` endpoints returning latest summaries with provenance; confirm via `tests/chat/test_summary_endpoints.py`.
  - **Subtask 6.4.c: Integrate summaries into retrieval ranking** – Update retrieval pipeline to inject summary nodes during hybrid search; validate via evaluation harness scenario `reports/eval/conversation_summary_uplift.md` showing recall improvements.
...
- **High-Level Task 8.4: Support multi-embedder strategy and cost controls**
  - **Subtask 8.4.a: Abstract embedder provider interfaces** – Implement `meshmind/embeddings/providers` module supporting OpenAI, Anthropic, Cohere, and Ollama backends with retry/circuit breaker policies; confirm via `tests/embeddings/test_provider_switching.py`.
  - **Subtask 8.4.b: Add dynamic provider routing** – Create policy engine `meshmind/embeddings/router.py` selecting providers based on tenant config, cost ceilings, and latency SLOs; verify via `tests/embeddings/test_router_policies.py` and load experiment logs `reports/backends/embedder_routing.md`.
  - **Subtask 8.4.c: Surface embedder cost dashboards** – Emit per-provider cost/latency metrics, update Grafana dashboard `dashboards/embeddings.json`, and document playbook `docs/backends/embedder_cost_controls.md`; confirm via screenshot artifact and finance review.
