...
- [ ] Implement dependency guards and lazy imports for optional packages (`mgclient`, `tiktoken`, `celery`, `sentence-transformers`).
- [ ] Add bootstrap helper for default encoder registration and call it from the CLI.
- [ ] Update OpenAI encoder implementation to align with latest SDK responses and retry semantics.
- [ ] Improve configuration guidance and automation for environment variables and service setup.
- [ ] Wire `EntityRegistry` and `PredicateRegistry` into the storage pipeline and client.
- [ ] Implement CRUD and triplet methods on `MeshMind`, including relationship persistence in `GraphDriver`.
