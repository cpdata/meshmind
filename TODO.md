...
- [ ] Draft `CLEANUP.md` outlining post-restriction cleanups for files that were temporarily modified to satisfy sandbox limitations.
- [ ] Audit the repository for direct `import openai` usage to scope the `llm_client` refactor.
- [ ] Implement a provider-agnostic `meshmind/llm_client.py` wrapper that routes requests via configurable endpoint URLs.
- [ ] Replace all direct OpenAI client interactions in the codebase with the new `llm_client` abstraction.
- [ ] Update unit tests and documentation to reflect the `llm_client` usage pattern.
