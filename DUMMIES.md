...
| gRPC stub implementation | `meshmind/api/grpc.py` (`GrpcServiceStub` + generated protobuf helpers) | Provides an in-process service implementation backed by the canonical proto schema so tests can exercise the service layer without spinning up gRPC infrastructure. | Unit tests and docs rely on the stub; production servers are still TODO. | Promote to a deployable gRPC server once infrastructure exists; keep the stub for tests but route it through the generated service classes (already implemented). |
