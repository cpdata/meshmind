...
- Developer-facing documentation now lives in `docs/` alongside the canonical `README.md` and must be updated with feature changes.
- Docker Compose now provisions Memgraph, Redis, and a Celery worker; see `ENVIRONMENT_NEEDS.md` for enabling optional
  services locally.

## Roadmap Highlights
- Push graph-backed retrieval deeper into the drivers (vector similarity, structured filters) to avoid loading entire namespaces into memory.
