...
- `meshmind.pipeline.store` and `meshmind.tasks.scheduled`: emit counters for stored items and maintenance plans. Consolidation
  retries now record `task.maintenance.retries`, `task.maintenance.retry_success`, and `task.maintenance.failures`, while
  `task.consolidate.failed_batches` gauges highlight batches that exhausted their retry budget.
