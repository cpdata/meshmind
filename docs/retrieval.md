...
fetches memories via `MemoryManager.list_memories(namespace, entity_labels)` before running the hybrid search.
...
- Graph wrapper `graph_vector_search` now filters by namespace and entity labels at the driver.
...
## Entity Label Filtering

All graph-backed helpers accept `entity_labels`. The labels are forwarded to `MemoryManager` and ultimately to the graph
driver so only relevant entity types are hydrated into Python. This prevents unnecessary deserialization when a graph
contains many heterogeneous memory types.
