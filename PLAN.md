...
3. **OpenAI SDK Compatibility** – Extraction and embedding adapters align with the Responses API; remaining polish tracked in
   `ISSUES.md`.
4. **Configuration Clarity** – `README.md` and `NEEDED_FOR_TESTING.md` document environment variables and service setup.
...
1. **Search Coverage** – Hybrid, vector-only, regex, exact-match, fuzzy, and LLM rerank helpers are implemented and exposed.
   Graph-backed wrappers now pull candidates from the configured driver when no list is provided. Next: push similarity scoring
