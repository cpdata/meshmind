...
4. **LLM Provider Flexibility** – Direct `openai` usage remains in the codebase. Upcoming work will add a configurable
   `llm_client` wrapper plus per-operation endpoint/model overrides so alternative providers (OpenRouter, Google) can be swapped
   in without touching downstream modules.
...
1. **Testing Overhaul** – Pytest suites rely on local fixtures, compatibility shims, and Celery workflow coverage. Graph-backed
   ...
   adding cross-backend integration coverage.
