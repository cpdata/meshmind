...
4. **LLM Provider Flexibility** – Direct `openai` usage has been consolidated behind `meshmind.llm_client`. The new wrapper
   honors `LLM_*` environment variables and CLI overrides for extraction, embeddings, and reranking. Next: extend service/API
   payloads to surface the same overrides once external integrations come online.
