...
class SQLiteGraphDriver(GraphDriver):
        ...
        *,
        ...
        if entity_labels:
            ...
            for idx, label in enumerate(entity_labels):
                ...
                params[key] = label
            clauses.append(f"label IN ({', '.join(placeholders)})")
        query = "SELECT * FROM entities"
        if clauses:
            query += " WHERE " + " AND ".join(clauses)
        query += " ORDER BY namespace, label, name"
    ...
    def search_entities(
        ...
        *,
        ...
        if entity_labels:
            ...
            clauses.append(f"label IN ({', '.join(placeholders)})")
        if query:
            params["needle"] = f"%{query.lower()}%"
            clauses.append(
                "(LOWER(name) LIKE :needle OR LOWER(props) LIKE :needle)"
            )
        sql = "SELECT * FROM entities"
        if clauses:
            sql += " WHERE " + " AND ".join(clauses)
        sql += " ORDER BY namespace, label, name"
        if limit is not None:
            if limit <= 0:
                return []
            sql += " LIMIT :limit"
    ...
    def count_entities(self, namespace: Optional[str] = None) -> Dict[str, Dict[str, int]]:
        ...
        params: Dict[str, Any] = {}
        sql = (
            "SELECT COALESCE(namespace, '') AS namespace, label, COUNT(*) AS count "
            "FROM entities"
        )
        if namespace:
            sql += " WHERE namespace = :namespace"
            params["namespace"] = namespace
        sql += " GROUP BY namespace, label"
        cur.execute(sql, params)
        rows = cur.fetchall()
