...
class MeshMind:
    ...
    def __init__(
        ...
        if embedding_model:
            config = config.override(models={"embedding": embedding_model})
    ...
    def list_memories(
        ...
        *,
        ...
        return self._ensure_manager().list_memories(
            namespace,
            entity_labels,
            offset=offset,
            limit=limit,
    ...
    def memory_counts(self, namespace: str | None = None) -> Dict[str, Dict[str, int]]:
        return self._ensure_manager().count_memories(namespace)
    ...
    def search(
        ...
        if cfg.rerank_model is None:
            cfg.rerank_model = self.llm_config.model_for("rerank", fallback=None)
        ...
        if use_llm_rerank:
            active_reranker = lambda q, c, k: llm_rerank(
                ...
                endpoint=self.llm_config.base_url_for("rerank"),
            )
