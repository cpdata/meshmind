...
class RestAPIStub:
    ...
    def dispatch(self, method: str, path: str, payload: Dict[str, Any] | None = None) -> Dict[str, Any]:
        ...
        if method == "GET" and path == "/memories":
            ...
            memories = self.service.list_memories(namespace, entity_labels)
...
def create_app(service: MemoryService) -> Any:
    ...
    @app.get("/memories")
    def list_memories(namespace: str | None = None, entity_labels: List[str] | None = None):
        memories = service.list_memories(namespace, entity_labels)
        return {"memories": [mem.dict() for mem in memories]}
