...
- `meshmind/tests`: Pytest suites rely on fixtures (`memory_factory`, `dummy_encoder`, in-memory drivers, service stubs) and
  pure-Python doubles (compatibility BaseModel, fake Memgraph/Redis/embedding/LLM clients), allowing the suite to run without Memgraph, OpenAI, or Redis dependencies. `test_setup_scripts.py` exercises the provisioning scripts in validation mode, `test_counts_smoke.py` covers REST/CLI count surfaces, and `test_tasks_scheduled.py` verifies maintenance backoff semantics.
