...
- `scheduled.py`: Defines periodic consolidation, compression, and expiry jobs that now initialize drivers and managers lazily,
  emit observability events, and tolerate missing dependencies during import.
...
- `meshmind/tests`: Pytest suites rely on fixtures (`memory_factory`, `dummy_encoder`, in-memory drivers, service stubs) and
  pure-Python doubles, allowing the suite to run without Memgraph, OpenAI, or Redis dependencies.

## External Dependencies
- Required: `openai`, `pydantic`, `pydantic-settings`, `numpy`, `scikit-learn`, `rapidfuzz`, `python-dotenv`, `pymgclient` (when using Memgraph).
...
## Operational Notes
- Graph persistence requires a configured backend: in-memory/SQLite need no services; Memgraph requires a running instance reachable via `settings.MEMGRAPH_URI` and `pymgclient`; Neo4j requires the official driver and credentials.
